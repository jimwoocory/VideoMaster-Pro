...
class YouTubeDownloaderApp:
    def __init__(self, root):
        self.root = root
        self.root.title("YouTube 下载器 V2.0")
        # 保持原始比例，适当增大
    ...
    def create_widgets(self):
        ...
        ttk.Label(url_frame, text="YouTube 链接:").grid(row=0, column=0, sticky=tk.W, pady=8)
        self.url_entry = ttk.Entry(url_frame, width=65)
        self.url_entry.grid(row=0, column=1, sticky=tk.W, pady=8, padx=(10, 5))
        self.url_entry.insert(0, "https://www.youtube.com/watch?v=")

        ttk.Button(url_frame, text="获取信息", command=self.fetch_video_info).grid(row=0, column=2, padx=(5, 0), rowspan=2, sticky=tk.NS)

        ttk.Label(url_frame, text="或批量输入URLs (每行一个):").grid(row=1, column=0, sticky=tk.W, pady=8)
        self.urls_text = scrolledtext.ScrolledText(url_frame, wrap=tk.WORD, height=4, width=65)
        self.urls_text.grid(row=1, column=1, sticky=tk.W, pady=8, padx=(10, 5))

        ttk.Label(url_frame, text="代理地址 (可选):").grid(row=2, column=0, sticky=tk.W, pady=8)
        self.proxy_entry = ttk.Entry(url_frame, width=65)
        self.proxy_entry.grid(row=2, column=1, sticky=tk.W, pady=8, padx=(10, 5))
        ...
        ttk.Label(path_frame, text="路径:").grid(row=0, column=0, sticky=tk.W, pady=8)
        self.save_path_var = tk.StringVar(value=os.getcwd())
        ttk.Entry(path_frame, textvariable=self.save_path_var, width=55).grid(row=0, column=1, sticky=tk.W, pady=8, padx=(10, 5))
        ttk.Button(path_frame, text="浏览", command=self.browse_save_path).grid(row=0, column=2, padx=(5, 0))
        ...
        ttk.Label(info_frame, textvariable=self.title_var).grid(row=0, column=0, sticky=tk.W, pady=5)
        ttk.Label(info_frame, textvariable=self.duration_var).grid(row=0, column=1, sticky=tk.W, pady=5, padx=(30, 0))
        ttk.Label(info_frame, textvariable=self.views_var).grid(row=1, column=0, sticky=tk.W, pady=5)
        ttk.Label(info_frame, textvariable=self.uploader_var).grid(row=1, column=1, sticky=tk.W, pady=5, padx=(30, 0))
        ...
        # 第一行选项
        ttk.Label(options_frame, text="自定义格式ID:").grid(row=0, column=0, sticky=tk.W, pady=8)
        self.format_id_var = tk.StringVar(value="bv*+ba/b")
        ttk.Entry(options_frame, textvariable=self.format_id_var, width=18).grid(row=0, column=1, sticky=tk.W, pady=8, padx=(10, 15))
        ttk.Button(options_frame, text="查询格式", command=self.query_formats).grid(row=0, column=2, padx=(0, 15))

        self.subtitle_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="下载字幕", variable=self.subtitle_var).grid(row=0, column=3, sticky=tk.W, pady=8, padx=(0, 15))

        ttk.Label(options_frame, text="线程数:").grid(row=0, column=4, sticky=tk.W, pady=8)
        self.threads_var = tk.StringVar(value="4")
        ttk.Combobox(options_frame, textvariable=self.threads_var, values=["1", "2", "4", "8", "16"], width=8).grid(row=0, column=5, sticky=tk.W, pady=8, padx=(10, 0))

        # 第二行选项
        self.transcode_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(options_frame, text="下载后转码", variable=self.transcode_var).grid(row=1, column=0, sticky=tk.W, pady=8, columnspan=2)
        self.transcode_format = tk.StringVar(value="mp4")
        ttk.Combobox(options_frame, textvariable=self.transcode_format, values=["mp4", "mkv", "avi", "mov", "webm"], width=12).grid(row=1, column=2, sticky=tk.W, pady=8, padx=(10, 0))

        # 按钮区域
        button_frame = ttk.Frame(main_frame)
        button_frame.pack(fill=tk.X, pady=(0, 10))

        ttk.Button(button_frame, text="开始下载", command=self.start_download, width=18).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(button_frame, text="终止下载", command=self.stop_download, width=18).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(button_frame, text="清空日志", command=self.clear_logs, width=18).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Button(button_frame, text="查看历史", command=self.show_history, width=18).pack(side=tk.LEFT, padx=(0, 10))
            ...
            try:
                ...
                if os.path.exists(ydl_path):
                    ...
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        # 生成格式信息
                        formats_info = f"可用格式 for: {info_dict.get('title')}\n"
                        for f in formats:
    ...
    def _download(self, task_id, url, proxy, save_path, format_id, download_subtitles, thread_count, transcode, transcode_format):
        ...
        try:
            ...
            if os.path.exists(ydl_path):
                ...
                while True:
                    ...
                    if output == '' and process.poll() is not None:
                        break
                    if output and not self.abort_all_tasks:
                        # 解析进度信息
                ...
                if return_code == 0 and not self.abort_all_tasks:
                ...
                elif self.abort_all_tasks:
                    self.result_queue.put(("info", "下载已取消"))
                else:
                    self.result_queue.put(("error", f"下载失败，返回代码: {return_code}"))
            else:
                # 回退到 Python 库
                import yt_dlp
            ...
            # 如果启用了转码，执行转码
            if transcode and not self.abort_all_tasks:
                self.result_queue.put(("info", "开始转码..."))
                # 转码逻辑这里简化处理
...
def show_splash_screen(root):
    ...
    label = tk.Label(splash, text="YouTube 下载器 V2.0", font=('Microsoft YaHei UI', 20, 'bold'), fg="red")
    label.pack(pady=30)
    ...
    status_label = tk.Label(splash, text="正在加载组件...", font=('Microsoft YaHei UI', 12))
    status_label.pack()
